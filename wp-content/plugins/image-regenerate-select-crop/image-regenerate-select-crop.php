 ...
 * Plugin URI: http://iuliacazan.ro/image-regenerate-select-crop/
 ...
 * License: GPL2
 *
 * Copyright (C) 2014-2017 Iulia Cazan
 *
...
define( 'SIRSC_ASSETS_VER', '20180127.1850' );
...
class SIRSC_Image_Regenerate_Select_Crop {
	...
	private function init() {
		...
		self::$exclude_post_type = array( 'nav_menu_item', 'revision', 'attachment' );
		if ( is_admin() ) {
		...
		add_filter( 'intermediate_image_sizes_advanced', array( get_called_class(), 'filter_ignore_global_image_sizes' ) );
	...
	public static function process_filtered_attachments( $meta_id = '', $post_id = '', $meta_key = '', $meta_value = '' ) {
		if ( ! empty( $post_id ) && '_wp_attachment_metadata' === $meta_key && ! empty( $meta_value ) ) {
			...
			if ( ! empty( self::$settings['force_original_to'] ) ) {
				...
				return self::process_image_resize_brute( $post_id, $size['width'], $size['height'] );
			}
		}
	}
	 ...
	 */
	public static function process_image_resize_brute( $id, $max_width, $max_height ) {
		self::load_settings_for_post_id( $id );
		self::make_images_if_not_exists( $id, 'all' ); // Make sure we get in the DB and folders all image sizes with the custom restrictions.
		...
		if ( ! is_wp_error( $img ) && ! empty( $m['file'] ) ) {
			...
			if ( ! empty( $max_width ) && ! empty( $max_height ) ) {
				if ( $size['width'] >= $size['height'] ) { // This is a landscape image, let's resize it by max width.
					if ( $size['width'] >= $max_width ) {
						$img->resize( $max_width, null );
						$to_replace = true;
					}
				} else { // This is a portrait image, let's resize it by max height.
					if ( $size['height'] >= $max_height ) {
						$img->resize( null, $max_height );
						$to_replace = true;
					}
				}
			} elseif ( ! empty( $max_width ) ) { // This is not restricted by height, but only by max width.
				if ( $size['width'] >= $max_width ) {
					$img->resize( $max_width, null );
					$to_replace = true;
				}
			} else { // This is not restricted by width, but only by max height.
				if ( $size['height'] >= $max_height ) {
					$img->resize( null, $max_height );
					$to_replace = true;
				}
	...
	public static function maybe_save_settings() {
		...
		if ( ! empty( $nonce ) ) {
			...
			wp_redirect( esc_url( self::$plugin_url ) . '&_sirsc_post_types=' . $post_types );
		}
	}
	...
	public static function image_regenerate_select_crop_settings() {
		...
		?>
		<br/><h1><?php esc_html_e( 'Image Regenerate & Select Crop Settings', 'sirsc' ); ?></h1><br/>
		...
		<table cellspacing="0">
			<tr>
				<td>
					<h3><a class="dashicons dashicons-info" title="<?php echo esc_attr( 'Details', 'sirsc' ); ?>" onclick="sirsc_toggle_info('#info_developer_mode')"></a> <?php esc_html_e( 'Images Placeholders Developer Mode', 'sirsc' ); ?> &nbsp;</h3>
					<div class="sirsc_info_box_wrap">
						<div id="info_developer_mode" class="sirsc_info_box" onclick="sirsc_toggle_info('#info_developer_mode')">
							<?php esc_html_e( 'This option allows you to display placeholders for front-side images called programmatically (that are not embedded in content with their src, but retrieved with the wp_get_attachment_image_src, and the other related WP native functions). If there is no placeholder set, then the default behavior would be to display the full size image instead of a missing image size.', 'sirsc' ); ?>
							<hr/><?php esc_html_e( 'If you activate the "force global" option, all the images on the front size that are related to posts will be replaced with the placeholders that mention the image size required. This is useful for debug, to quick identify the image sizes used for each layout.', 'sirsc' ); ?>
							<hr/><?php esc_html_e( 'If you activate the "only missing images" option, all the images on the front size that are related to posts and do not have the requested image size generate, will be replaced with the placeholders that mention the image size required. This is useful for showing smaller images instead of full size images.', 'sirsc' ); ?>
						</div>
					</div>
				</td>
				<td>
					<h3><label><input type="radio" name="_sirsrc_placeholders" id="_sirsrc_placeholders_none" value="" <?php checked( true, ( empty( $settings['placeholders'] ) ) ); ?> /> <?php esc_html_e( 'no placeholder', 'sirsc' ); ?></label></h3>
				</td>
				<td>
					<h3><label><input type="radio" name="_sirsrc_placeholders" id="_sirsrc_placeholders_force_global" value="force_global" <?php checked( true, ( ! empty( $settings['placeholders']['force_global'] ) ) ); ?> /> <?php esc_html_e( 'force global', 'sirsc' ); ?></label></h3>
				</td>
				<td>
					<h3><label><input type="radio" name="_sirsrc_placeholders" id="_sirsrc_placeholders_only_missing" value="only_missing" <?php checked( true, ( ! empty( $settings['placeholders']['only_missing'] ) ) ); ?> /> <?php esc_html_e( 'only missing images', 'sirsc' ); ?></label></h3>
				</td>
			</tr>
		</table>
		<hr/><h2><?php esc_html_e( 'Exclude Image Sizes', 'sirsc' ); ?></h2>
		<p><?php esc_html_e( 'The selected image sizes will be excluded from the generation of the new images. By default, all image sizes defined in the system will be allowed. You can setup a global configuration, or individual configuration for all images attached to a particular post type. If no particular settings are made for a post type, then the default general settings will be used (* means you added your settings for that option).', 'sirsc' ); ?></p>
		<?php
		if ( ! empty( $post_types ) ) {
			$ptypes       = array();
			$has_settings = ( ! empty( $default_plugin_settings ) ) ? '* ' : '';
			?>
			<select name="_sirsc_post_types" id="_sirsc_post_type" onchange="sirsc_load_post_type(this, '<?php echo esc_url( self::$plugin_url ); ?>')"><option value=""><?php echo esc_html( $has_settings . esc_html__( 'General settings (used as default for all images)', 'sirsc' ) ); ?></option>
			<?php
			foreach ( $post_types as $pt => $obj ) {
				...
				$has_settings = ( ! empty( $pt_s ) ) ? '* ' : '';
				?>
				<option value="<?php echo esc_attr( $pt ); ?>"<?php selected( 1, $is_selected ); ?>><?php echo esc_html( $has_settings . esc_html__( 'Settings for images attached to a ', 'sirsc' ) . ' ' . $pt . $extra ); ?></option>
				<?php
			}
			?>
			</select><hr />
		...
		<tr>
			<td>
				<h3><a class="dashicons dashicons-info" title="<?php esc_attr_e( 'Details', 'sirsc' ); ?>" onclick="sirsc_toggle_info('#info_global_ignore')"></a> <?php esc_html_e( 'Global Ignore', 'sirsc' ); ?></h3>
				<div class="sirsc_info_box_wrap">
					<div id="info_global_ignore" class="sirsc_info_box" onclick="sirsc_toggle_info('#info_global_ignore')">
						<?php esc_html_e( 'This option allows you to exclude globally from the application some of the image sizes that are registered through various plugins and themes options, but you don\'t need these in your application at all (these are just stored in your folders and database but not used).', 'sirsc' ); ?>
						<hr/><?php esc_html_e( 'By excluding these, the unnecessary image sizes will not be generated at all.', 'sirsc' ); ?>
					</div>
				</div>
			</td>
			<td>
				<h3><?php esc_html_e( 'Image Size Name', 'sirsc' ); ?></h3>
			</td>
			<td>
				<h3><?php esc_html_e( 'Image Size Description', 'sirsc' ); ?></h3>
			</td>
			<td>
				<h3><a class="dashicons dashicons-info" title="<?php esc_attr_e( 'Details', 'sirsc' ); ?>" onclick="sirsc_toggle_info('#info_exclude')"></a> <?php esc_html_e( 'Hide Preview', 'sirsc' ); ?></h3>
				<div class="sirsc_info_box_wrap">
					<div id="info_exclude" class="sirsc_info_box" onclick="sirsc_toggle_info('#info_exclude')">
						<?php esc_html_e( 'This option allows you to exclude from the "Image Regenerate & Select Crop Settings" lightbox the details and options for the selected image sizes.', 'sirsc' ); ?><hr/><?php esc_html_e( 'This is useful when you want to restrict from other users the functionality of crop or resize for particular image sizes', 'sirsc' ); ?>
					</div>
				</div>
			</td>
			<td>
				<h3><a class="dashicons dashicons-info" title="<?php esc_attr_e( 'Details', 'sirsc' ); ?>" onclick="sirsc_toggle_info('#info_force_original')"></a> <?php esc_html_e( 'Force Original', 'sirsc' ); ?></h3>
				<label><input type="radio" name="_sirsrc_force_original_to" id="_sirsrc_force_original_to_0" value="0" <?php checked( 1, 1 ); ?> /> <?php esc_html_e( 'nothing selected', 'sirsc' ); ?></label>
				<div class="sirsc_info_box_wrap">
					<div id="info_force_original" class="sirsc_info_box" onclick="sirsc_toggle_info('#info_force_original')">
						<?php esc_html_e( 'This option means that the original image will be scaled to a max width or a max height specified by the image size you select.', 'sirsc' ); ?><hr/><?php esc_html_e( 'This might be useful if you do not use the original image in any of the layouts at the full size, and this might save some storage space.', 'sirsc' ); ?><hr/><?php esc_html_e( 'Leave "nothing selected" to keep the original image as what you upload.', 'sirsc' ); ?>
					</div>
				</div>
			</td>
			<td>
				<h3><a class="dashicons dashicons-info" title="<?php esc_attr_e( 'Default Crop', 'sirsc' ); ?>" onclick="sirsc_toggle_info('#info_default_crop')"></a> <?php esc_html_e( 'Default Crop', 'sirsc' ); ?></h3>
				<div class="sirsc_info_box_wrap">
					<div id="info_default_crop" class="sirsc_info_box" onclick="sirsc_toggle_info('#info_default_crop')">
						<?php esc_html_e( 'This option allows you to set a default crop position for the images generated for particular image size. This default option will be used when you chose to regenerate an individual image or all of these and also when a new image is uploaded.', 'sirsc' ); ?><hr/><?php esc_html_e( 'The quality option is allowing you to control the quality of the images that are uploaded/generated. This can be useful for performance. However, please be sure not to change the quality of the full image, or of that set as the forced original. Setting a lower quality is recommended for smaller images sizes.', 'sirsc' ); ?>
					</div>
				</div>
			</td>
			<td>
				<h3><a class="dashicons dashicons-info" title="<?php esc_attr_e( 'Details', 'sirsc' ); ?>" onclick="sirsc_toggle_info('#info_clean_up')"></a> <?php esc_html_e( 'Cleanup All', 'sirsc' ); ?></h3>
				<div class="sirsc_info_box_wrap">
					<div id="info_clean_up" class="sirsc_info_box" onclick="sirsc_toggle_info('#info_clean_up')">
						<?php esc_html_e( 'This option allows you to clean up all the image sizes you already have in the application but you don\'t use these at all.', 'sirsc' ); ?><hr/><?php esc_html_e( 'Please be careful, once you click to remove the selected image size, the action is irreversible, the images generated will be deleted from your folders and database records.', 'sirsc' ); ?>
					</div>
				</div>
			</td>
			<td>
				<h3><a class="dashicons dashicons-info" title="<?php esc_attr_e( 'Details', 'sirsc' ); ?>" onclick="sirsc_toggle_info('#info_regenerate')"></a> <?php esc_html_e( 'Regenerate All', 'sirsc' ); ?></h3>
				<div class="sirsc_info_box_wrap">
					<div id="info_regenerate" class="sirsc_info_box" onclick="sirsc_toggle_info('#info_regenerate')">
						<?php esc_html_e( 'This option allows you to regenerate all the image for the selected image size', 'sirsc' ); ?><hr/><?php esc_html_e( 'Please be careful, once you click to regenerate the selected image size, the action is irreversible, the images already generated will be overwritten.', 'sirsc' ); ?>
					</div>
				</div>
			</td>
		</tr>
		...
		if ( ! empty( $all_sizes ) ) {
			foreach ( $all_sizes as $k => $v ) {
				...
				?>
				<tr class="<?php echo esc_attr( $use['line_class'] ); ?>">
					<td>
						<label><input type="checkbox" name="_sirsrc_complete_global_ignore[<?php echo esc_attr( $k ); ?>]" id="_sirsrc_complete_global_ignore_<?php echo esc_attr( $k ); ?>" value="<?php echo esc_attr( $k ); ?>" <?php checked( 1, $use['is_ignored'] ); ?> /> <?php esc_html_e( 'global ignore', 'sirsc' ); ?></label>
					</td>
					<td>
						<b><?php echo esc_html( $k ); ?></b><?php self::image_placeholder_for_image_size( $k, true ); ?>
					</td>
					<td><?php echo wp_kses_post( self::size_to_text( $v ) ); ?></td>
					<td>
						<label><input type="checkbox" name="_sirsrc_exclude_size[<?php echo esc_attr( $k ); ?>]" id="_sirsrc_exclude_size_<?php echo esc_attr( $k ); ?>" value="<?php echo esc_attr( $k ); ?>" <?php checked( 1, $use['is_checked'] ); ?> /> <?php esc_html_e( 'hide', 'sirsc' ); ?></label>
					</td>
					<td>
						<label><input type="radio" name="_sirsrc_force_original_to" id="_sirsrc_force_original_to_<?php echo esc_attr( $k ); ?>" value="<?php echo esc_attr( $k ); ?>" <?php checked( 1, $use['is_forced'] ); ?> /> <?php esc_html_e( 'force original', 'sirsc' ); ?></label>
					</td>
					<td>
						<?php if ( ! empty( $v['crop'] ) ) : ?>
							<?php esc_html_e( 'Position', 'sirsc' ); ?><?php echo wp_kses( str_replace( 'crop_small_type_' . $k . '"', '_sirsrc_default_crop[' . $k . ']" onchange="jQuery(\'#sirsc_settings_frm #submit\').trigger(\'click\');"', self::make_generate_images_crop( 0, $k, false, $use['has_crop'] ) ), $allow_html ); ?><hr>
						<?php endif; ?>
						<?php esc_html_e( 'Quality', 'sirsc' ); ?> <input type="number" name="_sirsrc_default_quality[<?php echo esc_attr( $k ); ?>]" id="_sirsrc_default_quality_<?php echo esc_attr( $k ); ?>" max="100" min="10" value="<?php echo (int) $use['quality']; ?>" size="2" style="width: 7em" onchange="alert('<?php esc_attr_e( 'Please be aware that your are changing the quality of the images going further for this images size!', 'sirsc' ); ?>'); jQuery('#sirsc_settings_frm #submit').trigger('click');">
					</td>
					<td>
						<?php $total_cleanup = self::calculate_total_to_cleanup( $_sirsc_post_types, $k ); ?>
						<?php if ( ! empty( $total_cleanup ) ) : ?>
							<span class="button-secondary button-large" title="<?php echo intval( $total_cleanup ); ?>" onclick="sirsc_initiate_cleanup('<?php echo esc_attr( $k ); ?>');"><b class="dashicons dashicons-update" title="<?php esc_attr_e( 'Cleanup All', 'sirsc' ); ?>"></b> <?php esc_html_e( 'Cleanup', 'sirsc' ); ?></span>
					...
					<td>
						<?php if ( empty( $use['is_ignored'] ) ) { ?>
							<span class="button-primary button-large" onclick="sirsc_initiate_regenerate('<?php echo esc_attr( $k ); ?>');"><b class="dashicons dashicons-update" title="<?php esc_attr_e( 'Regenerate All', 'sirsc' ); ?>"></b> <?php esc_html_e( 'Regenerate', 'sirsc' ); ?></span>
							<div class="sirsc_button-regenerate-wrap">
			...
			<tr>
				<td>
					<div class="updated">
						...
						<?php esc_html_e( 'To assure the optimal behavior for the features of this plugin, please make sure you visit and update your settings here, whenever you activate a new theme or plugins, so that the new image size registerd, adjusted or removed to be reflected also here.', 'sirsc' ); ?>
					</div>
				</td>
				<td nowrap="nowrap">
					<div class="floatright textright">
						<b><?php esc_html_e( 'Don\'t forget to donate !', 'sirsc' ); ?></b>
						<br/><?php esc_html_e( 'It means you apreciate the time I spent to develop the plugin for your benefit.', 'sirsc' ); ?>
						<br/><b><?php esc_html_e( 'Thank you !', 'sirsc' ); ?></b>
					</div>
				</td>
				<td>
	...
	public static function make_generate_images_button( $attachment_id = 0 ) {
		...
		<div class="sirsc_button-regenerate">
			<div id="sirsc_inline_regenerate_sizes' . (int) $attachment_id . '">
				<div class="button-primary button-large" onclick="sirsc_open_details(\'' . (int) $attachment_id . '\')"><div class="dashicons dashicons-format-gallery" title="' . esc_attr__( 'Details/Options', 'sirsc' ) . '"></div> ' . esc_html__( 'Details/Options', 'sirsc' ) . '</div>
					...
					if ( ! empty( $filename ) ) :
						...
						<?php if ( ! empty( $all_size ) ) : ?>
							<table class="wp-list-table widefat fixed media">
								<thead>
									<tr>
										<th colspan="3"><?php esc_html_e( 'The original image', 'sirsc' ); ?>: <b><?php echo (int) $original_w; ?></b>x<b><?php echo (int) $original_h; ?></b>px, <?php esc_html_e( 'file size', 'sirsc' ); ?>: <b><?php echo self::human_filesize( $original_filesize ); // WPCS: XSS OK. ?></b>.</th>
									</tr>
									<tr>
										<th class="manage-column"><b><?php esc_html_e( 'Image size details', 'sirsc' ); ?></b></th>
										<th class="manage-column textcenter"><b><?php esc_html_e( 'Image generated', 'sirsc' ); ?></b></th>
										<th class="manage-column textright"><b><?php esc_html_e( 'Actions', 'sirsc' ); ?></b></th>
									</tr>
								</thead>
								<tbody id="the-list">
							<?php
							...
							foreach ( $all_size as $k => $v ) {
								...
								if ( ! empty( $rez_img['found'] ) ) {
									...
									$im  = '<span id="idsrc' . intval( $post_data['post_id'] ) . $k . '"><img src="' . $ima[0] . '?v=' . time() . '" border="0" /><br /> ' . $rez_img['width'] . 'x' . $rez_img['height'] . 'px</span>';
								} else {
								...
								if ( ! empty( $rez_img['is_crop'] ) ) {
									if ( ! empty( $rez_img['can_be_cropped'] ) ) {
										$action .= '<div class="dashicons dashicons-image-crop"></div> ' . esc_html__( 'Crop image', 'sirsc' ) . '<div class="sirsc_clearAll"></div>' . self::make_generate_images_crop( $post_data['post_id'], $k ) . '';
									}
								}
								...
								if ( ! empty( $rez_img['can_be_generated'] ) ) {
									...
									$action .= esc_html__( 'Cannot be generated, the original image is smaller than the requested size.', 'sirsc' );
								}
								...
								echo '<tr class="' . $cl . '" id="sirsc_recordsArray_' . (int) $post_data['post_id'] . $k . '">
									...
									<td class="image-size-column"><b>' . $k . '</b><br />(' . $size_text . ')<hr>' . esc_html__( 'Quality', 'sirsc' ) . ' <b>' . $size_quality . '%</b><br>' . esc_html__( 'File size', 'sirsc' ) . ' <b class="image-file-size">' . self::human_filesize( $filesize ) . '</b></td>
									<td class="image-src-column"><div class="result_inline"><div id="sirsc_recordsArray_' . (int) $post_data['post_id'] . $k . '_result" class="result inline"><span class="spinner off"></span></div></div>' . $im . '</td>
				...
				if ( ! empty( $post ) ) {
					...
					if ( 'all' !== $sizes ) {
						...
						if ( ! empty( $tmp_details['crop'] ) ) {
							$crop_table = '<div class="dashicons dashicons-image-crop"></div>' . esc_html__( 'Crop image', 'sirsc' ) . '<div class="sirsc_clearAll"></div>' . preg_replace( '/[\x00-\x1F\x80-\xFF]/', '', self::make_generate_images_crop( intval( $post_data['post_id'] ), $sizes ) ) . '<div class="sirsc_clearAll"></div><a class="button" onclick="sirsc_start_regenerate(\'' . intval( $post_data['post_id'] ) . $sizes . '\');"><b class="dashicons dashicons-update"></b> Regenerate</a>';
						}
			...
			if ( 'all' === $selected_size ) {
				...
				if ( ! empty( $selected_size ) && ! empty( $alls[ $selected_size ] ) ) {
					...
					if ( ! empty( self::$settings['force_original_to'] ) && $selected_size === self::$settings['force_original_to'] ) {
						$size = self::get_all_image_sizes( self::$settings['force_original_to'] );
						self::process_image_resize_brute( $id, $size['width'], $size['height'] );
					}
					$sizes        = array(
	...
	public static function calculate_total_to_cleanup( $post_type = '', $image_size_name = '', $next_post_id = 0 ) {
		...
		if ( ! empty( $image_size_name ) ) {
			...
			$rows = $wpdb->get_results( $tmp_query, ARRAY_A );
			if ( ! empty( $rows ) && is_array( $rows ) ) {
	...
	public static function ajax_regenerate_image_sizes_on_request() {
		if ( ! empty( $_REQUEST['sirsc_data'] ) ) {
			...
			if ( ! empty( $post_data['_sisrsc_regenerate_image_size_name'] ) ) {
				...
				$rows = $wpdb->get_results( $wpdb->prepare( ' SELECT count(p.ID) as total_to_update FROM ' . $wpdb->posts . ' as p ' . $cond_join . ' WHERE p.ID > %d AND ( p.post_mime_type like %s OR p.post_mime_type like %s OR p.post_mime_type like %s )' . $cond_where . ' ORDER BY p.ID ASC ', (int) $next_post_id, 'image/gif', 'image/jpeg', 'image/png' ), ARRAY_A );
				if ( ! empty( $rows ) && is_array( $rows ) ) {
				...
				if ( $total_to_update > 0 ) {
					...
					<div class="sirsc_image-size-selection-box">
						...
						<div class="inside">';
					$rows = $wpdb->get_results( $wpdb->prepare( ' SELECT p.ID FROM ' . $wpdb->posts . ' as p ' . $cond_join . ' WHERE p.ID > %d AND ( p.post_mime_type like %s OR p.post_mime_type like %s OR p.post_mime_type like %s )' . $cond_where . ' ORDER BY p.ID ASC LIMIT 0, 1', (int) $next_post_id, 'image/gif', 'image/jpeg', 'image/png' ), ARRAY_A );
					if ( ! empty( $rows ) && is_array( $rows ) ) {
						foreach ( $rows as $v ) {
							echo '<center><hr />';
							...
							if ( ! empty( $filename ) && file_exists( $filename ) ) {
								...
								if ( ! empty( $th[0] ) ) {
									$th_src = $th[0];
									echo '<span><img src="' . $th_src . '?cache=' . time() . '" /><b class="spinner inline"></b></span><hr />' . $th_src; // WPCS: XSS OK.
								} else {
...
if ( defined( 'WP_CLI' ) && WP_CLI ) {
	...
	class SIRSC_Image_Regenerate_Select_Crop_CLI_Command extends WP_CLI_Command {
							...
							foreach ( $rows as $v ) {
								...
								if ( ! empty( $filename ) && file_exists( $filename ) ) {
									...
									if ( ! empty( $th[0] ) ) {
										...
										WP_CLI::error( esc_html__( 'Could not generate, the original is too small.', 'sirsc' ) );
									}
								} else {
									WP_CLI::error( esc_html__( 'Could not generate, the original file is missing ', 'sirsc' ) ) . $filename . ' !';
								}
								$progress->tick();
							}
							$progress->finish();
		...
		private function make_query( $post_type = '', $parent_id = 0, $action = 'REGENERATE' ) {
			...
			$query .= " WHERE ( p.post_mime_type = '%s' OR p.post_mime_type = '%s' OR p.post_mime_type = '%s' )";
			$args = array_merge( $args, array( 'image/gif', 'image/jpeg', 'image/png' ) );
			if ( ! empty( $post_type ) && '*' !== $post_type ) {
